...
from utils.utils import create_experiment
import mlflow.pytorch as mp

logger = logging.getLogger(__name__)
experiment_name = "yolov4_training"
run_name = "0"
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    with mlflow.start_run():
        ...
        run_params = {}
        with open('data/hyp.scratch.yaml', 'r') as yaml_file:
            try:
                run_params = yaml.safe_load(yaml_file)
        ...
        for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
            ...
            if rank in [-1, 0]:
                ...
                for x, tag in zip(list(mloss[:-1]) + list(results) + lr, tags):
                    print(f"x {x}, tag {tag}")
                    mlflow.log_metric(tag.replace(":"," "), x)
                    if tb_writer:
        ...
        # print(f"wdir {wdir}")
        # mlflow.log_artifact(wdir / 'best.pt')
        # mlflow.log_artifact(wdir / 'last.pt')
        # mlflow.log_artifact(results_file)
        wandb.run.finish() if wandb and wandb.run else None
        torch.cuda.empty_cache()
        run_params = {}
        for artifact in ['results.png']:
